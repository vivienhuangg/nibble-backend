...
export class SyncConcept {
  ...
  async synchronize(record: ActionRecord) {
    console.log("🔵 SYNC ENGINE: synchronize() called for action");
    ...
    console.log("🔵 SYNC ENGINE: Found", syncs ? syncs.size : 0, "syncs for this action");
    if (syncs) {
      for (const sync of syncs) {
        console.log("🔵 SYNC ENGINE: Checking sync:", sync.sync);
        let [frames, actionSymbols] = await this.matchWhen(
          record,
          sync,
        );
        console.log("🔵 SYNC ENGINE: matchWhen returned", frames.length, "frames for sync:", sync.sync);
        if (frames.length > 0) {
          this.logFrames(
          ...
          if (sync.where !== undefined) {
            console.log("🔵 SYNC ENGINE: Calling where() for sync:", sync.sync);
            const maybeFrames = sync.where(frames);
            frames = maybeFrames instanceof Promise
              ? await maybeFrames
              : maybeFrames;
            console.log("🔵 SYNC ENGINE: where() returned", frames.length, "frames");
            this.logFrames(`After processing \`where\`:`, frames);
          }
          console.log("🔵 SYNC ENGINE: Calling addThen for sync:", sync.sync);
  ...
  matchThen(then: ActionPattern, frame: Frame) {
    ...
    for (const [key, value] of Object.entries(then.input)) {
      if (typeof value === "symbol") {
        if (!(value in frame)) continue;
        const matchedValue = frame[value];
        if (matchedValue === undefined) {
          throw new Error(
            `Missing binding: ${String(value)} in frame: ${frame}`,
          );
        }
        bound.push([key, matchedValue]);
      } else {
